...
**II. 기술 구현**
[핵심 인프라](#core-infrastructure) · [데이터 엔지니어링](#data-engineering) · [모델 학습](#model-training)
...
- **명시적 상태 관리**를 위한 Project Compass (MCP)
...
| :---------------------------------- | :------- | :--------- |
| **단계 1-4:** 핵심 개발     | 완료 | 100%       |
| **단계 5:** 데이터 정제 & ETL  | 진행 중   | 90%        |
| **단계 6:** 아키텍처 업그레이드 | 진행 중   | 50%        |

**전체 진행률:** 85%

**현재 중점 사항:**
- **도메인 우선 아키텍처 리팩터링:** 소스 코드와 Hydra 설정을 도메인 중심 구조로 이동 (50% 완료).
- **에이전트 간 협업 프로토콜 (IACP):** 다중 에이전트 워크플로우 프로토콜 수립 (50% 완료).
- **모델 학습:** AI Hub 공공행정문서 데이터셋에서 PARSeq/CRNN 텍스트 인식 모델 학습.

---

## 시스템 아키텍처

이 시스템은 **에이전트 제어 계층**이 데이터 엔지니어링과 모델 학습을 위한 별도의 파이프라인을 조율하는 3계층 아키텍처를 사용합니다.
...
graph TD
    ...
    subgraph DataOps["데이터 엔지니어링 & ETL 파이프라인"]
    ...
    %% ML Core Layer
    subgraph MLCore["AI 모델 학습 코어"]
        Hydra["Hydra 설정"]:::storage
...
### Project Compass V2: Vessel AI 프로토콜

**문제:** AI 에이전트는 복잡한 코드베이스를 탐색하기 위해 광범위한 컨텍스트가 필요하며, 이는 비효율적인 토큰 사용과 환각(hallucination)을 초래합니다.

**해결책:** **Project Compass V2 (Vessel)**은 상태 관리를 위한 엄격한 스키마 기반 아키텍처를 제공합니다. 작업 단위에 대한 "펄스" 라이프사이클을 강제하고 아티팩트를 위한 깨끗한 "스테이징" 영역을 유지하여 컨텍스트 오염을 최소화합니다.
...
- **아티팩트 순도:** 스테이징 영역은 관련 파일만 인덱싱합니다.
- **프로토콜 강제:** CLI 도구(`compass`)를 통해 에이전트가 환경을 자체 관리할 수 있습니다.

### AST 디버그 툴킷 (ADT)

**문제:** 다중 아키텍처 딥러닝 프로젝트에서 Hydra 구성 우선순위를 디버깅하는 것은 악명 높게 불안정합니다.

**해결책:** Python AST 정적 분석을 통해 OmegaConf 병합 작업을 추적하여 구성 흐름에 대한 "X-레이 비전"을 제공합니다.

**기능:**
- `trace-merges`: 구성 우선순위 순서 식별
- `analyze-config`: 충돌하는 재정의 감지
...
*분석 결과 L175의 사용자 정의 재정의가 가장 높은 우선순위를 유지합니다.*

**영향:** 구성 디버깅 시간이 수시간에서 수분으로 단축되었습니다. 실제 해결 사례는 [BUG_003](docs/artifacts/bug_reports/2026-01-04_1730_BUG_003_config-precedence-leak.ko.md)을 참조하십시오.
...
**문제:** JSON 주석과 함께 600K+ 고해상도 이미지를 처리하면 파일 시스템 병목 현상이 발생하고 학습 중 효율적인 랜덤 액세스가 방해됩니다.
...
- **I/O 최적화:** 메모리 매핑 읽기로 학습 중 역직렬화 오버헤드 제거

**기술 구현:**
- **멀티프로세싱:** CPU 바운드 이미지 디코딩을 모든 코어에 병렬화
- **재개 가능성:** JSON 상태 파일이 장기 작업 진행 상황을 추적
- **데이터 무결성:** 트랜잭션 수준 일관성과 함께 원자적 쓰기

### 클라우드 네이티브 배치 처리

**모듈:** `aws-batch-processor`

**문제:** Document Parse API 요금 제한(무료 티어)으로 대규모 동기식 처리가 방해됩니다.
...
**영향:** 로컬 머신 가동 시간 요구 없이 밤새 5,000+ 문서 처리.
...
- **로깅:** Weights & Biases
- **구성:** 아키텍처별 재정의가 있는 구성형 Hydra 설정
- **품질 보증:** AgentQMS를 통한 자동 버그 보고
...
**문제:** 빠른 반복으로 인해 아티팩트(스크립트, 설정, 출력)가 대량으로 생성되어 체계적인 조직 없이는 관리하기 어려워집니다.
...
**컨텍스트:** 초기 접근 방식은 영수증에 대해 LayoutLMv3를 사용하여 KIE를 수행하고 Upstage KIE API 엔티티 레이블과 Document Parse API 경계 상자를 병합했습니다.
...
**컨텍스트:** PARSeq 학습이 `FPNDecoder`(레거시 DBNet 설정)를 `PARSeqDecoder` 대신 잘못 인스턴스화했습니다.

**근본 원인:** Hydra의 구성형 설정 시스템이 아키텍처 특정 재정의보다 높은 우선순위로 레거시 기본값(`train_v2` → `_base/model` → `dbnet`)을 병합했습니다.

**해결책:** 아키텍처 정의와 충돌할 때 레거시 구성 요소를 제거하기 위해 `_filter_architecture_conflicts()` 메서드 구현.

**검증:** AST 디버그 툴킷이 수정 후 올바른 우선순위 순서를 확인했습니다.
...
- 레이아웃 주석이 필요 없는 단순화된 학습 파이프라인

### 전환 2: 동기식 → 비동기식 배치 처리

**근거:** Document Parse API 요금 제한으로 대규모 동기식 처리가 방해되었습니다.

**결과:**
- 로컬 처리 수일 대비 밤새 5,000+ 문서 처리
- 로컬 머신 가동 시간 요구 사항 제거
...
**문제:** `cv2` 엣지/코너 감지와 수동 휴리스틱을 사용한 초기 시도는 신뢰할 수 없음이 입증되었습니다. 배경이나 문서 위치의 미세한 변화는 복잡한 조건부 로직을 필요로 하여 투영 보정 실패를 빈번히 초래했습니다.

**증거:**
- 초기 200개 이미지 테스트: **73% 성공률**(146/200)
- 실패 모드: "영역 손실 너무 큼"(감지된 영역이 이미지의 <50%)
...
**발견:** 텍스트 감지에 대한 정규화 영향을 조사하는 동안 표준 그레이스케일 전처리가 도움이 되었지만, **적응형 대비(CLAHE)를 사용한 세피아 색상 필터**가 저신뢰 영역에서 가장 큰 성능 향상을 제공했습니다.

**증거:**
- **엣지 개선:** 기준 대비 +164.0%(0% → 164%)
- **대비 향상:** 기준 대비 +8.2 포인트
- **중요한 뉘앙스:** 올바른 적갈색 틴트 매핑이 필수적이었으며, 초기 녹색 틴트는 엣지 선명도를 23% 덜 제공
...
**해결책:** 원시 엣지 감지를 하이브리드 파이프라인으로 교체: **RemBG(딥러닝)**를 사용한 초기 문서 마스킹 후 기하학적 검증을 통한 코너 피팅.

**구현:**
- **단계 1:** RemBG가 배경 노이즈에서 문서를 격리하는 이진 마스크 생성
- **단계 2:** 사용자 정의 엣지 감지가 평행성, 반대쪽 대칭성, 각도 합을 기반으로 코너 검증
- **단계 3:** Max-Edge 규칙과 Lanczos4 보간을 사용한 투영 변환

**결과:**
- **성공률:** 최악의 성능 이미지 25/25에서 100%
...
| **하이브리드 (RemBG + 주요 확장)** | **100%** (25/25)       | **44.3**     | **프로덕션 준비 완료**               |

*세피아 필터는 그레이스케일보다 엣지 감지를 개선했지만, 세피아+CLAHE는 최종적으로 제거되었습니다. RemBG 마스킹으로 인한 대비 개선.
...
Upstage Document OCR 콘솔에서 영감을 받은 프론트엔드 개념 증명.
...
| **관측 가능성** | W&B, AgentQMS, Project Compass (MCP)      |
| **AI 컨텍스트** | QWEN.md (in .qwen/), MCP, ADT             |

---

## AI 에이전트를 위한 프로젝트 컨텍스트

이 코드베이스를 사용하는 AI 에이전트는 다음 컨텍스트 파일을 참조하십시오:
