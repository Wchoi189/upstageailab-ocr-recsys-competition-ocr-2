...
**Root Cause**: Mask type mismatch in `decoder.py:105` - `tgt_key_padding_mask` was `bool` while `tgt_mask` was `float`. PyTorch 2.6's stricter SDPA kernel handling caused undefined behavior during multiprocessing.
...
**Verification**: Successfully ran training with `num_workers=2` - completed 100+ batches without crash.
