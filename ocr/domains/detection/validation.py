...
from ocr.core.data.schemas import CacheConfig, ImageData, ImageLoadingConfig, ImageMetadata
from ocr.core.interfaces.validation_models import DataItem, MapData, MetricConfig
...
class DatasetConfig(BaseModel):
    ...
    def normalize_extensions(cls, value: Any) -> list[str]:
        ...
        if is_config(value):
            # If a config object is passed, convert it to a list of extensions
            # This assumes the config object has a way to represent extensions,
            # or that it's a single string/list of strings.
            # The original code expects a list of strings or a single string.
            # We'll try to extract it if it's a dict-like config.
            value = ensure_dict(value).get("extensions", [])
            if not value: # If no 'extensions' key, or empty, fall back to default
                return [".jpg", ".jpeg", ".png"]
...
class CollateOutput(_ModelBase):
    ...
    def _check_metadata(
        ...
        for entry in value:
            if entry is None:
                normalized.append(None)
            elif isinstance(entry, ImageMetadata) or is_config(entry):
...
class LightningStepPrediction(_ModelBase):
    ...
    def _validate_metadata(cls, value: dict[str, Any] | ImageMetadata | None) -> dict[str, Any] | ImageMetadata | None:
        if value is None or isinstance(value, ImageMetadata) or is_config(value):
            return value
