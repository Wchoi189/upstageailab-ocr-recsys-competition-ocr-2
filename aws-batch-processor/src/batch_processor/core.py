...
API_URL_SUBMIT_DOCUMENT_PARSE = cfg["api"]["endpoints"]["document_parse"]["submit"]
API_URL_STATUS_DOCUMENT_PARSE = cfg["api"]["endpoints"]["document_parse"]["status"]
API_URL_PREBUILT_EXTRACTION = cfg["api"]["endpoints"]["prebuilt_extraction"]
...
class ResumableBatchProcessor:
        ...
        async with semaphore:
            ...
            try:
                ...
                if self.api_type == "prebuilt-extraction":
                    ...
                    if self.enhanced:
                        # Omit "nightly" unless specifically requested, just use "enhanced" mode if that's the key
                        # Or per legacy script: data["mode"] = "enhanced" (model might need to ideally be nightly but optional)
                        data.add_field('mode', 'enhanced')
                                                ...
                                                try:
                                                    ...
                                                    if len(poly) >= 3:
                                                        ...
                                                        if not text:
                                                            ...
                                                            # If still no text and we haven't checked content (e.g. content was not a dict/str in first place? Unlikely)
    ...
    async def process_dataset(
        ...
        # Process in batches with frequent checkpointing
        # Use smaller checkpoint interval (100 images) for better recovery
        ...
        while start_idx < total_images:
            ...
            chunk_results = await self.process_batch(chunk_df, dataset_name, 0)
            ...
            if self.checkpoint_dir and len(chunk_results) > 0:
                ...
                self.save_checkpoint(checkpoint_name, all_results)
                logger.info(f"✓ Checkpoint saved: {len(all_results)}/{total_images} images")
