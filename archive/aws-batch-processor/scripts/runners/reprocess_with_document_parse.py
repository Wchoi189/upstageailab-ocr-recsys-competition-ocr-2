#!/usr/bin/env python3
"""Reprocess datasets with Document Parse API."""
import sys
import asyncio
import argparse
import boto3
...
async def reprocess_dataset(dataset_name, batch_size=500, concurrency=3, resume=False, api_key_env="UPSTAGE_API_KEY"):
    """Reprocess a dataset with Document Parse."""
    print(f"\n{'='*80}")
    print(f"Reprocessing {dataset_name} with Document Parse API")
    print(f"Resume: {resume}")
    print(f"API Key Env: {api_key_env}")
    print(f"{'='*80}\n")

    input_file = Path(f"data/input/{dataset_name}.parquet")
    output_file = Path(f"data/output/{dataset_name}_dp.parquet")
    ...
    s3_client = None # Local only for now to avoid S3 errors

    # Create processor with Document Parse
    checkpoint_dir = Path(f"data/checkpoints/{dataset_name}_dp")
    processor = ResumableBatchProcessor(
        api_key=api_key,
        api_type="document-parse", # Document Parse Mode
        enhanced=False, # Standard mode unless specified otherwise (user didn't request enhanced)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Reprocess datasets with Document Parse API")
    parser.add_argument("--dataset", type=str, default="baseline_test", help="Dataset name so split")
