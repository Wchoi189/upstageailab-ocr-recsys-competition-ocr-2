...
- **STATUS:** In Progress
- **CURRENT STEP:** Phase 4, Task 4.1 – API Proxy & Auth
- **LAST COMPLETED TASK:** Phase 3 (Tasks 3.1 & 3.2) – Command Builder + Extract playground pages migrated
- **NEXT TASK:** Implement `/app/api` proxy routes and session strategy for console requests
...
8. [ ] **Task 4.2: Analytics & Compliance**
   - [ ] Integrate Google Tag Manager (GTM) with consent gating via Chakra banner
   - [ ] Document instrumentation events, push to `dataLayer`, and verify consent flows
...
**TASK:** Implement API proxy + auth (Task 4.1)

**OBJECTIVE:** Move data calls off the browser and into Next.js server routes/actions with a simple auth strategy so upcoming pages can rely on consistent `/app/api` endpoints.

**APPROACH:**
1. Add `/app/api/commands/*` routes that proxy to FastAPI using environment-configured credentials pulled from `.env`.
2. Integrate a lightweight NextAuth (or custom JWT) provider that stores tokens in HTTP-only cookies and exposes session context to client components.
3. Update `apps/playground-console` TanStack Query hooks to call the proxy endpoints, surfacing Chakra alerts for failures.

**SUCCESS CRITERIA:**
- Console network tab shows calls to `/app/api/...` only; FastAPI stays server-side.
- Auth/session info is available to TopNav (for CTA state) and to API hooks for automatic header injection.
