...
class SepiaEnhancer:
    ...
    def __init__(self, method: str = "classic"):
        ...
        """
        self.method = method
        self.valid_methods = ["classic", "adaptive", "warm", "contrast", "all"]
    ...
    def enhance(self, img: np.ndarray) -> dict[str, tuple[np.ndarray, dict]]:
        ...
        if self.method == "all":
            methods = ["classic", "adaptive", "warm", "contrast"]
        else:
            methods = [self.method]
        ...
        for method in methods:
            ...
            if method == "classic":
                ...
                result = self.sepia_contrast(img)
            else:
    ...
    def sepia_contrast(self, img: np.ndarray) -> np.ndarray:
        """
        Sepia with automatic contrast adjustment.

        Combines sepia tone with CLAHE (Contrast Limited Adaptive
        Histogram Equalization) for improved text clarity.
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="classic",
        choices=["classic", "adaptive", "warm", "contrast", "all"],
