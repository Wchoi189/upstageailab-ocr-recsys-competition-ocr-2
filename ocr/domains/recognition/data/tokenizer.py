...
# Key: (resolved_charset_path, max_len) -> ensures unique cache per config
...
class KoreanOCRTokenizer:
    ...
    def __init__(self, charset_path: str | Path, max_len: int = 25):
        ...
        Note: Prefer using get_or_create() to benefit from caching.
        ...
        self.charset_path = str(Path(charset_path).resolve())  # Store resolved path
    ...
    @classmethod
    def get_or_create(cls, charset_path: str | Path, max_len: int = 25) -> "KoreanOCRTokenizer":
        """
        Get cached tokenizer instance or create new if not exists.

        This method implements singleton pattern to avoid redundant tokenizer loading.
        Same (charset_path, max_len) parameters will return the same instance.
        ...
        Returns:
            Cached or newly created KoreanOCRTokenizer instance
        """
        # Resolve path to absolute for consistent caching
        resolved_path = str(Path(charset_path).resolve())
        cache_key = (resolved_path, max_len)

        if cache_key not in _TOKENIZER_CACHE:
            logger.debug(
                "Creating new tokenizer: %s, max_len=%d",
                resolved_path, max_len
            )
            _TOKENIZER_CACHE[cache_key] = cls(charset_path, max_len)
        else:
            logger.debug(
                "Reusing cached tokenizer: %s, max_len=%d",
                resolved_path, max_len
            )
