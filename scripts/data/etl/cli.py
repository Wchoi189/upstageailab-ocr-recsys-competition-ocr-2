...
from scripts.data.etl.core import LMDBConverter
import lmdb
import cv2
import numpy as np
...
def convert(
    ...
    converter = LMDBConverter(
        input_dir=str(input_dir), output_dir=str(output_dir), num_workers=num_workers, batch_size=batch_size, limit=limit
    )
    converter.run()
...
def inspect(
    ...
    with env.begin() as txn:
        num_samples_encoded = txn.get(b"num-samples")
