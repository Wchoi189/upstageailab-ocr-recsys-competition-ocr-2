...
class TestCoordinateTransformationCorrectness:
    ...
    def test_center_padding_has_translation(self):
        ...
        assert abs(inv_matrix[0, 2]) > 1e-6 or abs(inv_matrix[1, 2]) > 1e-6, (
            "Center padding should have non-zero translation"
        )
    ...
    def test_coordinate_roundtrip_top_left(self):
        ...
        assert np.allclose(recovered, original_point, atol=1.0), (
            f"Roundtrip failed: original={original_point[0]}, "
            f"recovered={recovered[0]}"
        )
    ...
    def test_no_negative_coordinates_top_left(self):
        ...
        assert np.all(transformed >= 0), (
            f"Negative coordinates detected: {transformed[transformed < 0]}"
        )
    ...
    def test_padding_position_mismatch_detection(self):
        ...
        assert np.any(translation_diff > 1e-6), (
            "Translation components should differ between padding positions"
        )
