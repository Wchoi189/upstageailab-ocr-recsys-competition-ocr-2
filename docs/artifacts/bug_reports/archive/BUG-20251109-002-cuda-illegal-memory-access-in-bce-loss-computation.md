---
title: "Bug 20251109 002 Cuda Illegal Memory Access In Bce Loss Computation"
...
- Moving operations to CPU is safe because we're just counting elements
- **Critical Issue:** Even `.cpu()` fails if CUDA memory is corrupted, suggesting corruption happens earlier
- **Next Steps:**
  ...
  5. Consider using `CUDA_LAUNCH_BLOCKING=1` to identify exact operation causing corruption
