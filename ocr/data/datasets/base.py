...
- Configuration schemas: ocr.data.datasets/schemas.py
- Utility functions: ocr.core.utils/{cache_manager,image_utils,polygon_utils}/
...
# - USE extracted utilities from ocr.core.utils.* modules
#
...
class ValidatedOCRDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> dict[str, Any]:
        ...
        # Fallback: Check if maps were generated by transform pipeline (on-the-fly generation)
        if prob_map is None:
             # Check for both singular and plural keys (transforms might use either)
             prob_map = transformed.get("prob_map") or transformed.get("prob_maps")
             thresh_map = transformed.get("thresh_map") or transformed.get("thresh_maps")

             # If found in transformed, ensure they are numpy arrays if not already (though usually tensors or arrays)
             # DataItem validator handles conversion, but we should be careful.
    ...
    def _load_image_data(self, filename: str) -> "ImageData":
        ...
        - Use utilities from ocr.core.utils.image_utils
        ...
        Related: ocr.core.utils/image_utils.py, ocr.core.utils/orientation.py
        """
...
#    - ocr.core.utils.cache_manager.CacheManager
#    - ocr.core.utils.image_utils.* functions
#    - ocr.core.utils.polygon_utils.* functions
#    - ocr.core.utils.orientation.* functions
#
...
#
# - Data Contracts: ocr/validation/models.py
# - Schemas: ocr.data.datasets/schemas.py
# - Configuration: configs/**/*.yaml
# - Utilities: ocr.core.utils/*/
