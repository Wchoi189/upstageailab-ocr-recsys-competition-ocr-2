...
SCHEMA_PATH = STANDARDS_DIR / "schemas" / "ads-header.json"
...
def validate_header(header: Dict[str, Any], schema: Dict[str, Any]) -> List[str]:
    ...
    if "tier" in header:
        ...
        if not isinstance(tier, int) or tier < 1 or tier > 4:
            errors.append(f"Invalid tier: {tier} (must be 1-4)")
...
def detect_cycles(
    ...
    """
    def dfs(node: str, visited: Set[str], rec_stack: List[str]) -> Optional[List[str]]:
        visited.add(node)
...
def compute_pulse_delta(
    old_registry: Optional[Dict[str, Any]],
    ...
    try:
        ...
        if not args.no_graph:
            ...
            if not args.dry_run:
                ...
                print(f"   ✓ Graph generated (dry-run, not saved)")
